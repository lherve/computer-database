					...
					<div id="pagination" class="pagination">
						<ul>
							<li class="prev ${page.previous.number eq page.number ? 'disabled' : '' }">
								<c:choose>
									<c:when test="${page.previous.number eq page.number}"><a>&larr; Previous</a></c:when>
									<c:otherwise><a href="?search=${param.search}&page=${page.previous.number}&s=${s }">&larr; Previous</a></c:otherwise>
								</c:choose>
							</li>
							<li class="current">
							...
							<li class="next ${page.next.number eq page.number ? 'disabled' : page.size > page.total ? 'disabled' : '' }">
								<c:choose>
									<c:when test="${page.next.number eq page.number}"><a>Next &rarr;</a></c:when>
									<c:otherwise><a href="?search=${param.search}&page=${page.next.number}&s=${s }">Next &rarr;</a></c:otherwise>
								</c:choose>
							</li>
						</ul>
		        	</div>
